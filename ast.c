...
void display(struct node *T,int indent){    //对抽象语法树先根遍历
  int i=1;
  struct node *T0;
  if (T)
	{
	...
	case PARAM_DEC:     printf("%*c类型：%s, 参数名：%s\n", indent,' ',  \
                            T->ptr[0]->type==INT?"int": (T->ptr[0]->type==CHAR?"char":"float"),T->ptr[1]->type_id);
                            break;
	...
	case RETURN:        printf("%*c返回语句：\n",indent,' ');
                            display(T->ptr[0],indent+3);
                            break;
        case BREAK:         printf("%*cbreak语句\n",indent,' ');
                            display(T->ptr[0],indent+3);
                            break;
        case CONTINUE:      printf("%*ccontinue语句\n",indent,' ');
                            display(T->ptr[0],indent+3);
                            break;
	...
	case WHILE:         printf("%*c循环语句：\n",indent,' ');
        ...
        case FOR:           printf("%*c循环语句：\n",indent,' ');
                            printf("%*c初始化语句：\n",indent+3,' ');
                            display(T->ptr[0],indent+6);      //显示初始化语句
                            printf("%*c循环条件：\n",indent+3,' ');
                            display(T->ptr[1],indent+6);      //显示循环条件
                            printf("%*c末尾循环体：\n",indent+3,' ');
                            display((T->ptr[2])->ptr[0],indent+6);      //显示尾部循环条件
                            printf("%*c循环体：\n",indent+3,' ');
                            display((T->ptr[2])->ptr[1],indent+6);      //显示循环体
                            break;
	...
	case DIV:
        ...
        case INC:
        case DEC:           //对应着Exp INC 或 Exp DEC的情况
		            if((T->ptr[0])!=NULL){  
		                printf("%*c%s\n",indent,' ',T->type_id);
		                display(T->ptr[0],indent+3);
		            }
                            //对应着INC Exp 或 DEC Exp的情况
		            else{                    
		                printf("%*c%s\n",indent,' ',T->type_id);
		                display(T->ptr[1],indent+3);
		            }
